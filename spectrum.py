...
class spectrum:
    def __init__(self,filename,m_start=None,m_stop=None):
	    ...
	    Returns: Pandas dataframe 'df' containing mass data and plot of full spectrum with fit range markers
	    """
	    df_uncut = pd.read_csv(filename, header = None, names= ['Mass [u]', 'Counts'], skiprows = 18,delim_whitespace = True,index_col=False,dtype=float)
	    df_uncut.set_index('Mass [u]',inplace =True)
	    plt.rcParams.update({"font.size": 15})
	    fig  = plt.figure(figsize=(20,8))
	    global df # dataframe containing mass data to fit
	    try: 
                df = df_uncut.loc[m_start:m_stop]
                plt.title('Spectrum with fit range markers')
	    except TypeError:
                df = df_uncut
                plt.title('Spectrum (fit full range)')
	    df_uncut.plot(ax=fig.gca())
	    plt.vlines(m_start,0,1.2*max(df['Counts']))
	    plt.vlines(m_stop,0,1.2*max(df['Counts']))
	    plt.yscale('log')
	    plt.ylabel('Counts')
	    plt.show() 

	##### Define internal routine for smoothing spectrum before peak detection(taken from https://scipy-cookbook.readthedocs.io/items/SignalSmooth.html)
	...
	def plot(df,title="",ax=None,yscale='log',peaks=None,vmarkers=None,thres=None,ymin=None):
	...
	def add_peak(peaks,x_pos,species="?",m_AME=None,m_AME_error=None):
	    ...
	    peaks.append(p)
	    ##### Helper function to sort list of peaks by marker positions 'x_pos'
	    def sort_x(peak): 
		return peak.x_pos  
