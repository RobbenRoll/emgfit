...
import mpmath as mp
np_exp = np.frompyfunc(mp.exp,1,1) # convert mp.exp function to numpy function, avoids error in lmfit.Model( ... ) 
np_erfc = np.frompyfunc(mp.erfc,1,1) # convert mp.exp function to numpy function, avoids error in lmfit.Model( ... )
...
def h_m_emg(x, mu, sigma, *t_args): 
    ...
    for i in range(t_order_m):
        eta_m = li_eta_m[i]
        tau_m = li_tau_m[i]
        h_m += np.float64( eta_m/(2*tau_m)*np_exp( (sigma/(np.sqrt(2)*tau_m))**2 + (x-mu)/tau_m )*np_erfc( sigma/(np.sqrt(2)*tau_m) + (x-mu)/(np.sqrt(2)*sigma) ))
    #print("h_m:"+str(h_m))
    return h_m
#h_m_emg = np.vectorize(h_m_emg)
...
def h_p_emg(x, mu, sigma, *t_args):
    ...
    h_p = 0.
    if np.abs((x-mu)/sigma) < 5: # only calculate values 5-sigma around centroid position mu
        for i in range(t_order_p):
            eta_p = li_eta_p[i]
            tau_p = li_tau_p[i]
            h_p += np.float64(eta_p/(2*tau_p)*np_exp( (sigma/(np.sqrt(2)*tau_p))**2 - (x-mu)/tau_p )*np_erfc( sigma/(np.sqrt(2)*tau_p) - (x-mu)/(np.sqrt(2)*sigma) ))
    else: # neglect values far from centroid
        h_p = 0
    return h_p
h_p_emg = np.vectorize(h_p_emg)
...
def h_emg(x, mu, sigma , theta, *t_args):
    ...
    t_args = (eta_p1, eta_p2, ...), (tau_p1, tau_p2, ...),  (eta_p1, eta_p2, ...), (tau_p1, tau_p2, ...):  
